...
export default function ProductCard({product} : {product: Product 
  ...
  return (
    ...
    >
      <Link href={`/product/${product?.slug?.current as string}`}>
      {/* Image Section */}
      ...
      <div className="p-4">
        ...
        <div className="flex mt-2">
          <Rating name="half-rating-read" defaultValue={4.5} value={product?.averageRating} precision={0.5} size="small" readOnly />
          <p className="text-sm text-gray-500 ml-4">({product.stock})</p>
        </div>

          <PriceView price={product?.price as number} discount={product?.discount} className="mt-2"/>
