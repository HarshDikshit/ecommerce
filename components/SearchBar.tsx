...
const mockProducts = [
  {
    ...
    images: [{ asset: { url: 'https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=300' } }],
  ...
  },
  {
    ...
    images: [{ asset: { url: 'https://images.unsplash.com/photo-1515562141207-7a88fb7ce338?w=300' } }],
  ...
  },
  {
    ...
    price: 199.99,
    discount: 15,
    description: 'Beautiful Rose Quartz necklace for attracting love and harmony',
    images: [{ asset: { url: 'https://images.unsplash.com/photo-1599643478518-a784e5dc4c8f?w=300' } }],
  ...
  },
  {
    ...
    price: 299.99,
    discount: 20,
    description: 'Golden Pyrite mala beads for attracting wealth and prosperity',
    images: [{ asset: { url: 'https://images.unsplash.com/photo-1506629905496-00be5b89ff18?w=300' } }],
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          {product.status?.slice(0, 1).map((status: string) => (
                            <span key={status} className="inline-flex items-center px-2 py-1 rounded-md bg-green-100 text-green-800 text-xs font-medium">
                              {status}
                            </span>
                          ))}
                        </div>
                        ...
                        <div className="flex items-center gap-1 mb-2">
                          {product.bead?.slice(0, 2).map((bead:string) => (
                            <span key={bead} className="inline-flex items-center px-2 py-1 rounded-md bg-purple-100 text-purple-800 text-xs">
                              <Tag className="w-3 h-3 mr-1" />
                              {bead}
                            </span>
                          ))}
                          {product.purpose?.slice(0, 2).map((purpose: string) => (
                            <span key={purpose} className="inline-flex items-center px-2 py-1 rounded-md bg-orange-100 text-orange-800 text-xs">
                              {purpose}
                            </span>
                          ))}
                        </div>
          ...
          {!query && (
            <div className="px-4 py-6 border-t border-gray-200 bg-gray-50">
              ...
              <div className="text-sm text-gray-600 space-y-1">
                <p>• Search by product name: "Rudraksha", "Rose Quartz"</p>
                <p>• Find by purpose: "peace", "wealth", "protection"</p>
                <p>• Filter by bead type: "karungali", "pyrite", "sandalwood"</p>
                <p>• Browse categories: "meditation", "bracelets", "necklaces"</p>
              </div>
            </div>
          )}
        </div>
...
const SearchDialogHandler = () => {
  ...
  return (
    <div>
            <Search onClick={() => setIsSearchOpen(true)} className="w-5 h-5 hover:text-[var(--saffron)]" />
