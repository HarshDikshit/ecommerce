...
const WishlistProducts = () => {
    ...
    const handleResetWishlist = () => {
        const confirm = window.confirm("Are you sure wanted to reset wishlisht items?")
        if (confirm) {
            resetFavorite();
  ...
  return (
    <Container>
        {(favoriteProduct?.length > 0 )? (
                ...
                <table className='w-full border-collapse'>
                    <thead className='border-b'>
                        <tr className='bg-black/5'>
                            ...
                            <th className='p-2 text-left hidden md:table-cell'>Bead</th>
                            ...
                            <tr key={product?._id} className='border-b'>
                                <td className='px-2 py-4 flex items-center gap-2'>
                                    <X 
                                        ...
                                        {product?.images && <Link href={{pathname: `/product/${product?.slug?.current as string}`}} className='border rounded-md group hidden md:inline-flex'>
                                        <Image 
            ...
            {favoriteProduct?.length > 0 && (
                <Button className='mb-5 font-semibold w-fit' size={'sm'} variant={'destructive'} onClick={handleResetWishlist}>
                    Reset Wishlist
                </Button>
            )}
            </>
